...
The following are the improvements I made to the schemaand I will focus on 3 tableas examples:

### Table *'district'*

- I split the table in 3 different entities: *district, region and demographics*. This helps to reference the same information in different places in the database such as address, loans etc. This is in compliance of the 2dn Normal Form (2NF).
- Each table has a Primary key to uniquely identify the rows to achive Referencial Integrity.
<br></br>
...
- In the original table there was a "virtual" relationship between the tables "client", "district" and "disp" but not reenforced in the database design. There was no primary keys or foreign keys references. The disp table describes the types of customer that bank holds, Owner or Disponent. I have changed the name of this table to "client_status" for better undertanding. 
- The table "address" was non existent in the previous design. There was a column called "district_id" in the "client" table but again, no a foreign key. This table  It hold the addresses of the customers and any other address such as Branch addresses, other banks addresses among others.
- This led me to also create "phone" and "postcode" as separate entities that have a close relashionship with "address". This new primary keys can be referenced in many other systems such as insurance or any other activities to avoid data redundancy.
<br></br>
![image](https://user-images.githubusercontent.com/63274055/147274843-4fe59efa-b053-4e78-8f8e-00e9e792c238.png)
